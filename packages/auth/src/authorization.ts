...
export const ACTIONS = [
   ...
   *
   * ⚠️ **`customer.view` と分けてある。** その方が何を買ったかを見ることと、
   * **連絡先そのものを読み取ること**は、別の力である。前者が漏れても
   * 「誰の」が分からないが、後者はその 1 行だけで人に届く。
   *
   * ⚠️ **`auditor` には渡さない。** 監査は「運営が何をしたか」を見る仕事で、
   * 連絡先を読む必要は無い。**むしろ、読んだことを確かめる側**である。
   *
   * ⚠️ **この操作でも保存はしない**（`UD-503` 維持）。見るたびに認証基盤へ
   * 問い合わせ、返ってきた値は応答に載せて捨てる。DB には入らない。
   *
   * ⚠️ **使うたびに監査ログへ残る。** 「誰が・いつ・どのアカウントの分を」
   * まで。**アドレスの値そのものは残さない**——残せば、監査ログの側から
   * `UD-503` を破ることになる。
   */
  'customer.view_email',
  /**
  ...
  */
  ...
  'payout.mark_paid',
  /**
   * 作家さまのお振込先を、伏せずに読む（決定 2026-08-21）。
   *
   * ⚠️ **`payout.view` と分けてある。** いくら払うかを見ることと、
   * **どこへ振り込むかを読み取ること**は別の力である。前者が漏れても
   * 口座は分からないが、後者はその 1 行で金融機関へ届く。
   *
   * ⚠️ **`auditor` には渡さない。** 監査は「運営が何をしたか」を見る
   * 仕事で、口座番号を読む必要は無い。**むしろ、読んだことを確かめる側**
   * である（`customer.view_email` と同じ考え方）。
   *
   * ⚠️ **オーナー限定にしていない。** 振込は月次の実務で、**押すたびに
   * オーナーを待たせると、結局「オーナーの資格情報を借りる」運用が
   * 生まれる**。借りられた資格情報は `payout.mark_paid` まで通ってしまう
   * ので、絞ったつもりで広げることになる。代わりに、**読むたびに監査
   * ログへ残す**。
   *
   * ⚠️ **再認証（`RequireFreshAuth`）も課していない。** 5 分の窓では
   * 月末にまとめて振り込む作業が途中で切れる。切れると
   * **「一度読んで表計算へ写す」**という、いちばん避けたい運用へ倒れる。
   *
   * ⚠️ **値そのものは監査ログへ残さない。** 残せば、包んで保管した意味が
   * 監査ログの側から失われる。残すのは「誰が・いつ・どの精算のために」まで。
   */
  'payout_account.view_full',
  /**
   * 運営の売上レポートを見る（`UD-123` の一部・2026-08-22）。
   *
   * ⚠️ **`auditor` にも渡す。** 「いくら売れて、いくら返したか」は監査の
   * 対象そのものである。出るのは**期間ごとの合計**で、誰が買ったかも
   * 何を買ったかも含まない。
   *
   * ⚠️ **`buyer` には渡さない。** 会員は誰でも出品できる（`UD-806`）が、
   * ご自分の売上は `creator.earnings.view_own` で見る。ここに出るのは
   * **場全体の数字**である。
   */
  'sales_report.view',
  /**
   * 作家さまの一覧・詳細を見る（`UD-124` の一部・2026-08-22）。
   *
   * ⚠️ **`auditor` にも渡す。** 「誰にいくら払うことになっているか」を
   * 追えないと監査にならない。出るのは合計と件数までで、**お振込先の値も
   * ご連絡先も含まない**。
   *
   * ⚠️ **止める力を含まない。** 作家さま単位で出品を止める操作は、
   * まだ作っていない（何が起きるかを決めていない）。**この権限に
   * 後から足さないこと**——見る力と止める力は分けて配る。
   */
  'creator.view',
  /**
...
const ROLE_ACTIONS: Readonly<Record<Role, readonly Action[]>> = {
  ...
  operator: [
    ...
    'customer.view',
    'customer.note',
    // ⚠️ **日常の問い合わせ対応で使う。** オーナーの印は要求しない。
    //    印を要ると、応対中にオーナーを待つことになり、結局
    //    「オーナーの資格情報を借りる」運用が生まれる。
    'customer.view_email',
    'customer.email_change',
    'collection.view',
    'mint_job.retry',
    ...
    'payout.mark_paid',
    /*
      ⚠️ **運営だけ。** 振り込むために口座番号の全体が要るのは、実際に
         振込を行う人である。読むたびに監査ログへ残る。
    */
    'payout_account.view_full',
    // 会計へ渡す数字（`UD-123`）。⚠️ 期間ごとの合計まで。
    'sales_report.view',
    // 作家さまの一覧・詳細（`UD-124`）。⚠️ 見るだけ。止める口は無い。
    'creator.view',
  ...
  auditor: [
    ...
    /*
      ...
      ⚠️ **ご連絡先（`customer.view_email`）も渡さない。** 監査は
         **読んだことを確かめる側**であって、読む側ではない。
    */
    'collection.view',
    'audit_log.view',
    ...
    */
    ...
    //    出るのは期間ごとの合計で、誰が買ったかは含まない。
    'sales_report.view',
    // ⚠️ 誰にいくら払うことになっているかを追えないと監査にならない。
    'creator.view',
    // ⚠️ いくら誰へ払ったかが見えないと監査にならない。締めることはできない。
    // ⚠️ **お振込先（`payout_account.view_full`）は渡さない。** どこへ
    //    払ったかを確かめるのに口座番号は要らず、監査は**読んだことを
    //    確かめる側**である。
    'payout.view',
  ],
};
