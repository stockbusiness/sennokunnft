...
describe('注文の組み立て', () => {
  it('買える出品なら注文を組み立てる', () => {
    ...
    if (!result.ok) return;
    expect(result.value.amounts.totalAmount).toBe(1000);
    expect(result.value.items).toHaveLength(1);
    expect(result.value.currency).toBe('JPY');
  });

  /*
    ⚠️ **1 注文 1 作品 1 クリエイター**（指示書 §3-4）。
       明細が 1 本であることを、ここで固定しておく。
  */
  it('明細は必ず 1 本', () => {
  ...
  it('注文のクリエイターは明細のクリエイターと一致する', () => {
    const result = attempt();
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.creatorAccountId).toBe(CREATOR);
      expect(result.value.items[0]?.creatorAccountId).toBe(CREATOR);
    }
  });
  ...
  it('金額は単価 × 数量（整数計算）', () => {
    const result = attempt({ quantity: 3 });
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.value.amounts.subtotalAmount).toBe(3000);
  });

  it('手数料率を注文へ焼き付ける', () => {
    const result = attempt({ platformFeeRateBps: 1000 });
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.amounts.platformFeeRateBps).toBe(1000);
      expect(result.value.amounts.platformFeeAmount).toBe(100);
      expect(result.value.amounts.creatorAmount).toBe(900);
    }
  });

  /*
    ⚠️ **作った直後は、何も始まっていない。**
       ここを `pending` などで埋めると、始まっていないものが進行中に見える。
  */
  it('4 つの状態は「まだ何も起きていない」から始まる', () => {
    const result = attempt();
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.orderStatus).toBe('pending');
      expect(result.value.paymentStatus).toBe('not_started');
      expect(result.value.fulfillmentStatus).toBe('not_started');
      expect(result.value.refundStatus).toBe('none');
    }
  });
  ...
  describe('スナップショット', () => {
    it('作品名を注文時点の値で持つ', () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) expect(result.value.items[0]?.titleSnapshot).toBe('当時の名前');
    });
    ...
    it('単価を注文時点の値で持つ', () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) expect(result.value.items[0]?.unitPrice.amountMinor).toBe(777);
    });
  });
  ...
  describe('予約の期限', () => {
    it('既定は 30 分後', () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value.reservationExpiresAt.getTime()).toBe(
    ...
    it('呼び出し側で変えられる', () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value.reservationExpiresAt.getTime()).toBe(NOW.getTime() + 5 * 60_000);
      }
    });
  });
});
