...
describe('知らせの種別', () => {
  it('すべての種別に対象の種類が決まっている', () => {
    ...
    /*
      ⚠️ **数を書いておく。** 種別を足すのは「人へ届く経路が増える」という
         こと。ここが落ちて手が止まるのは、そのための仕掛けである。

      ⚠️ **足すときは、文面の既定版も同じマイグレーションで入れる。** 種別
         だけ足すと、送る段になって「文面が無い」で止まる。止まった知らせは
         誰にも気づかれないまま溜まる。

      11 → 14（方針整理 2026-08-22）: 返金の申請と審査で 3 つ増えた。
      うち `refund_inquiry.asked` は**購入者ではなく作家さま**へ届く
      ——この語彙で初めて、宛先が買った方以外になった。
    */
