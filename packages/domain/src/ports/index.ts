...
/**
 * メールアドレスを照合用の値へ変換する口（`UD-121`）。
 *
 * ⚠️ **本システムは購入者のメールアドレスを平文で持たない**（`UD-503`）。
 * それでも問い合わせでは「このアドレスの人の注文はどれか」を引く必要がある。
 * そこで、保存するのは変換後の値だけにし、照合は
 * 「聞いたアドレスをその場で変換して一致を探す」形で行う。
 *
 * ⚠️ **素のハッシュで実装しない。** メールアドレスは取りうる値が狭く、
 * よくあるアドレスを並べた表と突き合わせれば元に戻せる。それでは
 * 平文を持たない意味が無い。鍵（pepper）付きの HMAC で実装すること。
 *
 * ⚠️ **鍵が無い配備では `null` を返す。** 鍵が無いからといって素のハッシュへ
 * 落とさない。落とすと、鍵がある配備と無い配備で保護の強さが変わり、
 * どちらで作られた行なのか見分けが付かないまま同じ列に混ざる。
 */
export interface EmailHashPort {
  /**
   * 照合用の値を返す。変換できない配備では `null`。
   *
   * ⚠️ 実装は引数をログへ出さないこと。
   */
