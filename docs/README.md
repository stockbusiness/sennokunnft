...
✅ **事実:** 本プロジェクトは新規リポジトリ `team478a/sengoku-nft-marketplace` を対象として指示された。

✅ **事実:** 実装セッションからは `team478a` organization へアクセスできず（別ownerのリポジトリ追加が拒否された）、
当該リポジトリの作成・push ができなかった。このため成果物は既存リポジトリ内の
**独立したサブツリー `sengoku-nft-marketplace/`** として作成している。

- 既存システム（Sengoku Market）のファイルは**1件も変更していない**
- ルートの `package.json` / `tsconfig.json` / 既存 CI とは分離しており、相互に影響しない
- 本サブツリーは `git subtree split --prefix=sengoku-nft-marketplace` で
  そのまま新リポジトリへ移送できる。移送手順は
  [OPERATIONS_AND_ROLLBACK.md](./OPERATIONS_AND_ROLLBACK.md) 「リポジトリ移送」を参照
