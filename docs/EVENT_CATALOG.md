...
> ⚠️ **未実装のギャップ2件。** `entitlement.granted` を実装する前に埋める。
>
> | 項目         | 状態                                                                                    |
> | ------------ | --------------------------------------------------------------------------------------- |
> | `image_hash` | **`artworks` に列が無い。** `image_key` / `image_content_type` / `image_byte_size` のみ |
> | `image_url`  | **出せない。** 現在はローカル保存で、Wallet が取得できる URL が無い（`UD-508`）         |
>
> どちらも本文書には書いてあるが実装が無い。**文書が先行している状態**なので、
> 送信側を作るときに必ず突き合わせること。
