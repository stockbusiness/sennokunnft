...
> ⚠️ **金額を含めない。** `order.paid` と同じく注文IDだけを載せる。
> 読む側は権限付きAPIで引く。イベントは複数の購読者・ログ・キューを
> 経由するため、載せた値はこちらの管理を離れる。

**積む条件（実装 2026-08-22）:**

|          |                                                                             |
| -------- | --------------------------------------------------------------------------- |
| いつ     | 返金が成立し、注文が**全額返金**になったとき                                |
| どこ     | `refunds.settle` の**同じトランザクションの中**                             |
| 何回     | 注文につき **1 回**（部分 UNIQUE 索引 `outbox_events_order_refunded_once`） |
| 一部返金 | **積まない**                                                                |

⚠️ **一部返金では積まない。** 一部返金は「返金された注文」ではない。
積むと、受け取る側が売上を丸ごと取り消す判断をしうる。

⚠️ **積めなくても返金は成立させる。** 索引違反は `ON CONFLICT DO NOTHING`
で握りつぶす。ここで例外を出すと、**決済事業者へ届いている返金の記録ごと
巻き戻る**——出来事を 1 件取りこぼすより悪い。

> ⚠️ **`outbox_events` には、いま送信側が無い。** `order.paid` も
> `order.refunded` も**行が溜まるだけ**である。代理店の受け口の契約は
> 未確定で（`UD-1008` は `common-users/resolve` までしか定めていない）、
> `UD-1003`（代理店報酬を本システムの販売に適用するか）も未決である。
> **こちらで契約を発明すると、あとで作り直しになる。**
>
> それでも積むのは、**そのとき積むはずだった出来事は作り直せない**ため。
> `refunds` を読めば「いつ返金になったか」は組み直せるが、対の
> `order.paid` / `order.refunded` が同じ日付から揃っている状態は、
> あとから作れない。
...
| 一部返金           | **自動処理しない。** 記録のみ残す                         |
| 争いの申し立て     | **記録するだけ。** 受取権も返金も動かさない（2026-08-22） |
| 争いの決着（勝訴） | **記録するだけ。** 返金にしない                           |
| 争いの決着（敗訴） | **返金として記録する。** 全額なら受取権も取り消す         |

> ⚠️ **争いが起きたことと、返金されたことは別である。** 申し立てで受取権を
> 取り消すと、**こちらが勝ったときに返せない**——外部のウォレットへ渡した
> ものは、こちらからは戻せない。詳細は `docs/CHARGEBACK_FLOW.md`。
>
> ⚠️ **敗訴では `charge.refunded` が届かない。** 事業者は返金（Refund）を
> 作らない。`charge.dispute.closed` で記録しなければ、どこにも残らない。
