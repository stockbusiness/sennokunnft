...
⚠️ **消す前に、上の 3-5 と 3-6 を必ず済ませる。順番を逆にすると管理画面へ入れなくなる。**

出品者向けの画面（`/creator`）では、ログイン済みの人がいるとこのトークンは
**使われない**。一方、管理画面（`/admin`）は、ログインしている人が `operator`
でなかったときにだけ、もう一度このトークンで試す。
つまり `operator` のアカウントが 1 つも無い状態でこのトークンを消すと、
**誰も管理画面を開けなくなる。**

`operator` を用意したあとは消してよい。残しておくと、`SUPABASE_*` を外した
ときに黙って「全員が同じ人」へ戻る経路が残る。
