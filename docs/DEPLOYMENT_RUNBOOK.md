...
#### 決済の鍵を世代として取り込む（`UD-128`。**決済を使うなら必須**）

⚠️ **2026-08-20 から、決済は世代の表の鍵で動く。** 環境変数
（`STRIPE_SECRET_KEY` / `STRIPE_WEBHOOK_SECRET`）に鍵が入っていても、
**世代を取り込んでいない配備では支払い口が作れない**。

これは意図した挙動である。落ちる形にすると、管理画面で世代を有効化した
つもりで環境変数の古い鍵が使われ続け、**入金先がずれてから気づく**ことになる。

起動時のログに次が出ていたら、この作業がまだである。

```
受付中の決済資格情報の世代がありません。支払い口は作れません。
```

**手順**（api のコンテナ内、または同じ `DATABASE_URL` を持つ環境で）:

```bash
pnpm payment:credential -- --import          # 配備環境の鍵を世代として取り込む
pnpm payment:credential -- --list            # id と状態を確認する
pnpm payment:credential -- --activate=<id>   # 接続確認を通してから受付を開始する
```

⚠️ **`--activate` は接続確認を通らないと成功しない。** 鍵の打ち間違いを
ここで止めるためで、不具合ではない。

⚠️ **鍵を入れ替えたあとも、旧世代を消さない。** 旧アカウントで発生した
決済の知らせは届き続け、**返金も旧世代の鍵でしかできない**。
退役（`retire`）はするが、行は残す。

⚠️ **2026-08-20 以降、これは実際の動作になった**（`UD-120`）。返金は
`payments.credential_id` の世代を開いて投げる。**開けなければ返金を断る**
（`REFUND_CREDENTIAL_UNAVAILABLE`）——受付中の世代へ落ちると、無関係の
アカウントから現金が出ていくため。旧世代の行を消すと、その世代で受けた
決済は**二度と返金できない**。

⚠️ **`PAYMENT_EMERGENCY_CREDENTIAL_OVERRIDE=true` は復旧専用。** 世代の表を
壊した場合に、配備環境の鍵で一時的に動かすためのもの。立てているあいだは
起動のたびに警告が出る。**上書き中の決済には世代が付かず、あとで返金できない**
ので、復旧したら必ず `false` へ戻す。

#### 問い合わせでメールから注文を辿れるようにする（`UD-121`）

⚠️ **この設定を入れないと、メールアドレスから注文を探せない。** 画面は
「この環境ではメールアドレスからのお調べができません」と出す。
**不具合ではない**——鍵の無い保護を装わないための動作である。

⚠️ **平文のメールアドレスを保存するための設定ではない**（`UD-503`）。
保存するのはこの鍵で作った照合値だけで、元のアドレスには戻せない。

```bash
# 32 文字以上の乱数を作って入れる。⚠️ この値をどこかに書き写さない。
fly secrets set --app sennokunnft-api EMAIL_LOOKUP_PEPPER="$(openssl rand -base64 48)"
```

⚠️ **照合値が付くのは、各利用者が次にログインした時点。** 設定を入れた
直後は、まだ誰も引けない。既存の利用者を一括で埋める方法は**無い**
（平文を持っていないため）。

⚠️ **入れ替えると、それ以前の照合値は引けなくなる。** 付け直せない。
**入れ替えるのは、鍵が漏れたときだけ**にする。

#### 問い合わせに答えるときの手順（対応する方へ）

1. 注文番号をうかがえたら、`/admin/orders` の「注文をお探しする」に入れる。
   **末尾 8 文字だけでも探せる**（お電話ではそこしか控えられないことが多い）。
2. 番号が分からないときは、**お申し込み日の範囲・金額・作品名**で絞る。
3. それでも見つからないときに、**「メールアドレスからさがす」**を使う。
   ⚠️ ここに入れたアドレスは保存されない。画面を閉じれば残らない。
4. 対応の内容は、注文の詳細にある**対応メモ**へ残す。
   ⚠️ **あとから消せない・直せない。** 訂正は新しいメモで行う。
   ⚠️ **メモにメールアドレスを書かない。** 保存しない決まりのため、
   書こうとすると保存を断られる。
...
fly secrets set --app sennokunnft-worker \
  ...
  INTERNAL_JOB_BASE_URL="http://sennokunnft-api.internal:8080/api/v1/internal/jobs" \
  INTERNAL_JOB_TOKEN="<api と同じ値>"
fly deploy --config fly.worker.toml --remote-only
```

GitHub のリポジトリ変数 `DEPLOY_WORKER` を `true` にして、自動デプロイへ載せる。

⚠️ **worker 用の Fly の合言葉を先に登録しておく。** api 用に発行した
合言葉は worker アプリには通らず、デプロイが `unauthorized` で落ちる。
**api だけ成功して worker だけ落ちる**ので、worker のコードが悪いと
読み違えやすい。§2-8 の `FLY_API_TOKEN_WORKER` をご覧いただきたい。
...
**フラグを OFF のままでも、秘密情報は 3 本要る**（2026-08-22 に 1 本から
増えました）。ほかは既定値で動く。フラグを ON にするときに、その段の
分だけ足す。

⚠️ **「フラグが両方 OFF なら worker は何もしない」は、もう正しくない。**
`2026-08-22` に worker が 2 つの役目を持ちました。フラグとは無関係に動きます。

1. **期限切れのお取り置きを解放する。** 止めてよい仕事ではない。止めると、
   決済されなかった枠が永久に押さえられ、**在庫があるのに買えなくなる**
2. **API の内部ジョブを定時に叩く**（5 本）。詳しくは §4-9

⚠️ **先に全部入れない。** 使っていない接続先や鍵を置いておくと、
「設定してあるから繋がっているはず」という誤解が生まれる。
実際に繋がるのはフラグを ON にした時なので、**設定と実態がずれる**。
ただし下の A は、フラグに関わらず**必ず要る**。
...
| `INTERNAL_JOB_BASE_URL`   | `fly secrets`     | `http://sennokunnft-api.internal:8080/api/v1/internal/jobs`（§4-9） |
| `INTERNAL_JOB_TOKEN`      | `fly secrets`     | ⚠️ **api と同じ値**（違うと 401 を受け続ける）                      |
...
⚠️ **`INTERNAL_JOB_*` の 2 本が無くても worker は起動する。** 止めると、
設定済みだったウォレットへのお届けまで巻き添えになるため。代わりに
起動ログへ警告を出す（§4-9 の確かめ方）。**起動したことを「動いている」
と読まないこと。**
...
## 4-9. 時計仕掛けを動かす（2026-08-22）

⚠️ **これを設定するまで、5 つの仕事が一度も動きません。** しかも
**エラーは出ません**——いちばん困る壊れ方です。

### 何が動いていないか

API には内部ジョブの口が 7 本あります。そのうち 2 本
（お取り置きの解放・ウォレットへのお届け）は worker が同じ待ち行列を
直接掃いているので動きます。**残りの 5 本には叩き手がいません。**

⚠️ **お取り置きの解放は、`INTERNAL_JOB_*` が無くても動きます。** worker が
直接掃くためです。動かないのは下の 5 本です。

| 仕事                     | 動かないと何が起きるか                                           |
| ------------------------ | ---------------------------------------------------------------- |
| 受取権の発行             | 決済直後の発行に失敗した注文が、**永久に受け取れない**           |
| 取消の知らせの補完       | 返金で取り消した作品が、相手側に**残ったまま**                   |
| 知らせの送信             | 「お支払いが済みました」が**一通も届かない**                     |
| 改定のお知らせの積み込み | 法務文書を改定しても、**誰にも知らせが行かない**                 |
| 運営への異常の知らせ     | **異常が起きても誰も気づかない**（知らせの仕組み自体が動かない） |

### 設定

worker に 2 つの環境変数を入れます。**両方そろって初めて動きます。**

```bash
# api と同じ合言葉を worker にも入れる
flyctl secrets set -a <workerのアプリ名> \
  INTERNAL_JOB_BASE_URL="http://<apiのアプリ名>.internal:8080/api/v1/internal/jobs" \
  INTERNAL_JOB_TOKEN="<api と同じ値>"
```

⚠️ **`INTERNAL_JOB_TOKEN` は api と worker で同じ値にします。** 違うと
worker は 401 を受け取り続け、ログには出ますが**仕事は一つも進みません**。

⚠️ **Fly.io の内部網（`.internal`）を使います。** 公開の URL でも動きますが、
内部ジョブの口が外から見える必要はありません。
...
```
API の内部ジョブを定時に叩きます   jobCount=5
```

これが出ていれば持っています。出ずに次が出ていたら、**設定できていません**。
...
⚠️ **`/admin/operations` という画面はありません。** 運営の状況は
**`/admin`（管理画面の最初のページ）** にあります。

「今日の動き」に、時計仕掛けの一覧が 7 本すべて出ます。
**「まだ一度も成功していません」** と出ている行があれば、その仕事は
まだ一度も成功していません。

⚠️ **worker が直接掃く 2 本も、ここに出ます（2026-08-22 に直しました）。**
以前は掃いていても記録を残さなかったため、**動いているのに「まだ一度も
成功していません」が永久に出続けていました**。いまは掃いた側が記録を
書きます。

⚠️ **ウォレットへのお届けは、`WALLET_DELIVERY_ENABLED` を下ろしている
あいだ記録が増えません。**「止めている」のであって「壊れている」のでは
ありません。フラグを下ろしたまま運用する予定なら、この行が進まないのは
正常です。
...
| 仕事                     | 間隔    | なぜ                                                         |
| ------------------------ | ------- | ------------------------------------------------------------ |
| 知らせの送信             | 1 分    | 遅いと、届く前に問い合わせが来る                             |
| 受取権の発行             | 5 分    | 取りこぼしの受け皿。ふだんは決済直後に発行される             |
| 運営への異常の知らせ     | 5 分    | 巡回を長くすると、設定した繰り返し間隔より粗くしか鳴らせない |
| 取消の知らせの補完       | 15 分   |                                                              |
| 改定のお知らせの積み込み | 24 時間 | 公開の直後にその場で積まれる。取りこぼしだけ拾う             |

⚠️ **間隔は「試みた時刻」から数えます。** 成功した時刻ではありません。
成功でしか進めないと、落ち続けている口を巡回のたびに叩き、相手が
復旧しかけたところへ集中して押し寄せます。

⚠️ **worker を再起動すると、次の巡回で全部が一度走ります。** どれも
冪等なので害はありませんが、デプロイ直後にログが賑やかになります。

### 記録を書くのは誰か

| 仕事                                                  | 掃くところ | 記録するところ |
| ----------------------------------------------------- | ---------- | -------------- |
| お取り置きの解放                                      | worker     | worker         |
| ウォレットへのお届け                                  | worker     | worker         |
| そのほか 5 本（発行・取消の補完・知らせ・改定・異常） | API        | API            |

⚠️ **記録は「入口」ではなく「仕事」に付いています。** 新しい時計を足す
ときは、**掃く側が記録も書く**ようにしてください。入口だけが記録する形に
戻すと、worker が掃く分の記録が落ち、**動いているのに永久に赤い行**が
できます。消えない色は読み飛ばされ、本当に止まった日にも気づけません。
...
**推奨は管理画面から**（`/admin/integrations/payment`）。環境変数は
**引き継ぎ元**として残してあり、管理画面で保存するまではそちらが使われる。

管理画面から設定する利点:

- 鍵の差し替えに**再起動が要らない**。次の呼び出しから効く
- **誰がいつ替えたかが監査ログに残る**。環境変数には残らない
- 取り違えを**保存の時点で断る**（`sk_live_` を staging へ、など）
- **止められる**。事故のとき、配備をやり直さずにその場で決済を止められる

手順:

1. `/admin/integrations/payment` を開く
2. 戻り先の URL と手数料率を保存する
3. シークレットキーと Webhook 署名シークレットを登録する（2 本とも要る）
4. 「接続を確認」を押す（`https://api.stripe.com` への到達性）
5. それぞれの鍵を「有効にする」
6. 連携を「有効にする」

⚠️ **4 の確認で分かるのは到達性まで。** 鍵が正しいかどうかは、実際に
支払いが通るまで分からない。

⚠️ **手数料率 0 のままでは有効にできない。** 0 は「無料」ではなく
「まだ決めていない」。
...
管理画面の「お支払いの設定」に表示される。DB に鍵を保存していれば DB、
していなければ配備時の設定。⚠️ **DB 側で「停止」したときは、環境変数へは
落ちない。** 落ちると、止める操作が効かないことになる。
