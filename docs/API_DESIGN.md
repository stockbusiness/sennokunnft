...
✅ **事実:** Phase 1 で実装するエンドポイントは **ヘルスチェックのみ**。
本書の他エンドポイントは Phase 2 以降の契約定義であり、実装しない。
...
```json
{
  "error": {
    ...
    "message": "在庫が不足しています",
    "details": [
      { "field": "quantity", "issue": "exceeds_available" }
    ],
    "requestId": "01J8Z7..."
  }
}
...
🟡 **仮決定:** 状態を変える POST は `Idempotency-Key` ヘッダを**必須**とする。

- 同一キー＋同一リクエスト本文 → 前回の結果を再返却（201/200 と同じ本文）
- 同一キー＋**異なる**リクエスト本文 → `409 IDEMPOTENCY_CONFLICT`
- キーの有効期間は 🟡 24時間
...
```json
{
  "status": "degraded",
  "checks": [
    { "name": "database", "status": "fail", "durationMs": 5001 }
  ]
}
```
