    ...
    AppModule.register({
      ...
      */
      ...
      operations: {
        ...
        /*
          止めている時計仕掛け（2026-08-22）。

          ⚠️ **フラグを下ろしているあいだ、お届けの心拍は増えない。**
             そのままでは「まだ一度も成功していません」の黄色が永久に
             残る。消えない警告は読み飛ばされ、**本当に止まった日にも
             気づけなくなる**。止めているものは灰色にする。

          ⚠️ **溜まっている数は隠れない。** 「ウォレットへのお届け待ち」は
             別の項目として出続ける。隠れるのは心拍だけである。

          ⚠️ **本番販売ガードには効かない。** 画面の色を変えても
             「発行・お届けの時計」の条件は緩まない。
        */
        pausedJobKeys: env.WALLET_DELIVERY_ENABLED ? [] : ['deliver-entitlements'],
        /*
          運営への知らせ（`UD-1102` の一部）。
