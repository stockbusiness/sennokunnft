...
export const OPERATIONS_COPY = {
  ...
  */
  /*
    ⚠️ **直せるようになったあとも、直し方を書いておく**（2026-08-24 決定）。
       押す人が「一括で片付けたい」と思ったときに、なぜ 1 件ずつなのかが
       その場で読めないと、別の口を作る話になる。
  */
  reservedCountRepairNote:
    '⚠️ 直せるのは 1 件ずつです。まとめて直す口は作っていません。ずれは同じ原因で何件も出ることがあり、まとめて押すと「本人が確かめていない判断」が一度に通ってしまうためです。何度も押すことになったら、それ自体が原因を調べる合図です。',
  reservedCountRepairTitle: '原因が分からないまま直した記録',
  reservedCountRepairDescription:
    'お取り置きの数を直したものの、なぜずれたのかがまだ分かっていない記録です。原因が分かったら、内容を書いて閉じてください。',
  reservedCountRepairNone: '原因の分からないまま直した記録はありません。',
  reservedCountRepairNoneHint:
    '直したものはすべて、原因が分かったうえで直したか、原因を突き止めて閉じてあります。',
  reservedCountRepairTruncated: '件数が多いため、一部だけ表示しています。',
  /*
    ⚠️ **「消える」と読ませない。** 閉じても、直したときの数と内訳は
       記録として残る。残ることがこの表を持つ理由そのものである。
  */
  reservedCountRepairResolveNote:
    '⚠️ 閉じても、直したときの数と内訳は記録として残ります。消える操作ではありません。',
} as const;
