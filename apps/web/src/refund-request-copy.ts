...
export const REFUND_REQUEST_COPY = {
  ...
  /*
    ⚠️ **既定を選んだ状態で出す。** 実務では事由の表に当てはまらないことが
       起きるので選べるようにしたが、**何もしなければ表どおり**になる形を
       保つ。既定を空にすると、毎回選ばせることになって表の意味が薄れる。
  */
  bearerHint:
    '事由から既定が決まります。こちらの不具合・カード会社からの申し立て・例外としてお通しする場合は、運営が被ります（作家さまの売上からは引きません）。実情に合わないときは変更できます。',
  bearerDefaultSuffix: '（事由からの既定）',
  /*
    ⚠️ **変えたことが記録に残る、とその場で伝える。** 黙って残すと、
       あとで証跡を見た運営が「監視されていた」と感じる。先に書いておく。
  */
  bearerOverrideHint:
    '既定と違う負担者をお選びになった場合、その事実が経過と操作の記録に残ります。理由は下の「判断の記録」へお書きください。',
  bearerOverriddenBadge: '既定から変更',
