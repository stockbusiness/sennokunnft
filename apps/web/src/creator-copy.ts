...
export const CREATOR_COPY = {
  ...
  */
  ...
  /*
    ...
    ⚠️ **「カード会社」までにとどめる。** どなたのご注文かは書かない。
       作家さまが買った方を探しに行く道を作らない。
    ⚠️ **「来月これだけ入ります」と読ませない。** 負ければ返金になり、
       お支払いは発生しない。額は出すが、確約はしない。
    ⚠️ **作家さまの落ち度ではないと明示する。** 何も書かないと、ご自分に
       非があったのかとご心配をおかけする。
  */
  earningsDeferredDisputeNotice: (count: number, amount: number): string =>
    `カード会社との確認中のため、${count.toLocaleString('ja-JP')} 件（${amount.toLocaleString('ja-JP')} 円）を今回は含めていません`,
  earningsDeferredDisputeHint:
    'カードをお使いになった方とカード会社との間で確認が進んでいるご注文です。確認が終わったあとの月に含めます。作家さまに落ち度があったということではありません。なお、確認の結果によってはご返金となり、その分のお支払いは発生しません。',
