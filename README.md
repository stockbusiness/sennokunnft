...
| **管理画面（作品の登録・編集・公開停止・削除）**              | —                                |
| **会員登録とログイン（メールのマジックリンク）**              | —                                |
| **作家の自己出品（`/creator`）**                              | —                                |
| **スタッフの招待と権限管理（オーナー制）**                    | —                                |
| 環境変数の型検証、構造化ログ                                  | —                                |
...
> 管理画面から、作品の登録・編集・画像の差し替え・公開／公開停止・削除が行える。
> 削除は公開中でなく、お支払い待ちと発行済みが 0 件のものだけ（`UD-113`）。
>
> 会員はメールのマジックリンクでログインし、自分の作品を出品できる（`UD-102` 決定変更）。
> スタッフの招待と権限変更はオーナーだけが行える（`UD-803`）。
...
⚠️ **`pnpm dev:api` は `tsc` でビルドしてから走らせる。** `tsx` は使わない。
esbuild 系の変換器は `emitDecoratorMetadata` を出力せず、Nest が依存を
解決できないまま**起動に成功し、全エンドポイントが 500 を返す**（起動ログは
正常に見える）。いまは起動時の検査が止めるので、替えるとその場で分かる。
詳しくは `docs/DEPLOYMENT_RUNBOOK.md` §7-1。

最初のビルドが終わるまで数秒かかる。`最初のビルドを待っています…` と出て
いるあいだは、まだ受け付けていない。
